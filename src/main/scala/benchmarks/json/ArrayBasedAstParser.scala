...
class JsonParser(input: ParserInput) {
  ...
  private[this] var fieldBuffer = new Array[JField](32)
  private[this] var elemBuffer = new Array[JValue](32)
  ...
  private def `object`(): Unit = {
    ...
    jValue = if (cursorChar != '}') {
      @tailrec def fields(ix: Int): Int = {
        `string`()
        require(':')
        ws()
        val key = sb.toString
        `value`()
        if (ix == fieldBuffer.length) {
          val newBuffer = new Array[JField](fieldBuffer.length << 1)
          System.arraycopy(fieldBuffer, 0, newBuffer, 0, ix)
          fieldBuffer = newBuffer
        }
        fieldBuffer(ix) = JField(key, jValue)
        if (ws(',')) fields(ix + 1) else ix
      }
      val count = fields(0)
      require('}')
      val fieldsArray = new Array[JField](count)
      System.arraycopy(fieldBuffer, 0, fieldsArray, 0, count)
      JObject.create_unsafe(fieldsArray)
    } else {
      advance()
      JObject.empty
    }
  ...
  private def `array`(): Unit = {
    ws()
    jValue = if (cursorChar != ']') {
      @tailrec def elements(ix: Int): Int = {
        `value`()
        if (ix == elemBuffer.length) {
          val newBuffer = new Array[JValue](elemBuffer.length << 1)
          System.arraycopy(elemBuffer, 0, newBuffer, 0, ix)
          elemBuffer = newBuffer
        }
        elemBuffer(ix) = jValue
        if (ws(',')) elements(ix + 1) else ix
      }
      val count = elements(0)
      require(']')
      val elementsArray = new Array[JValue](count)
      System.arraycopy(elemBuffer, 0, elementsArray, 0, count)
      JArray.create_unsafe(elementsArray)
    } else {
      advance()
      JArray.empty
    }
